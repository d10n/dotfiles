...
HOSTCOLORNUMBER=$(hosthash=${HOSTHASH:0:6} color_count=$COLORCOUNT python -c '
import os
import colorsys
...
snaps = [int(round(0xff * i / 6. + 0xff / 6. / 2.)) for i in (1, 2, 3, 4, 5)]
...
def hl2short(h, l, color_count):
    ...
    return h + 8 if l > .6 and color_count == 16 else h
color_count = int(os.environ["color_count"])
if color_count >= 256:
    print(rgb2short256(*rgb))
else:
    print(hl2short(h, l, color_count))
')
HOSTBGCOLOR=$(tput setab $HOSTCOLORNUMBER)
HOSTFGCOLOR=$(tput setaf $HOSTCOLORNUMBER)
if (( COLORCOUNT == 256 && 0x${HOSTHASH:4:2} < 0x5f )) ||
   # special case for blue because it's dark   HUE                         HUE                         LIGHTNESS
   (( COLORCOUNT == 256 && 0x${HOSTHASH:0:2} > 0x95 && 0x${HOSTHASH:0:2} < 0xc0 && 0x${HOSTHASH:4:2} < 0xc0 )) ||
   (( COLORCOUNT == 16 && HOSTCOLORNUMBER == 4 )) ||
   (( COLORCOUNT == 8 && HOSTCOLORNUMBER == 4 )); then
    # dark host color, light text color
    if (( COLORCOUNT == 8 )); then
        HOSTFGTEXT=$(tput setaf 7)
        HOSTBGTEXT=$(tput setab 7)
    else
        HOSTFGTEXT=$(tput setaf 15)
        HOSTBGTEXT=$(tput setab 15)
    fi
else
    # light host color, dark text color
    HOSTFGTEXT=$(tput setaf 0)
    HOSTBGTEXT=$(tput setab 0)
fi
